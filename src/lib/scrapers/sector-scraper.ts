...
export async function scrapeSectoralPE(): Promise<{ data: SectorPE[]; date: string }> {
  try {
    ...
    throw new Error(`Failed to scrape sectoral PE: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function scrapeSectorStocks(areaId: string): Promise<{ data: SectorStock[]; date: string; sectorName: string }> {
  try {
    ...
    throw new Error(`Failed to scrape sector stocks: ${error instanceof Error ? error.message : String(error)}`);
  }
}
